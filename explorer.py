...
Collections = namedtuple("Collections", ("people", "proposals"))


def cook(dev, proposals):
    p = {k: dev.get(k) for k in FIELDS}
    p["id"] = str(dev["_id"])
    contacted = False
    responded = False
    for email in dev["email"]:
        try:
            responded |= proposals[email["address"]]
            contacted = True
        except KeyError:
            continue
...
def lookup_developer():
    ...
    emails = list(chain.from_iterable((
        e["address"] for e in n["email"]) for n in neighbors))
